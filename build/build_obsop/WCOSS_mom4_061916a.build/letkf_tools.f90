...
!===============================================================================
  ...
  USE params_model, ONLY: iv2d_mld                                  !STEVE: for assimlating mixed layer depth
  USE params_model, ONLY: iv2d_sst, iv3d_u, iv3d_v, iv3d_t, iv3d_s  !STEVE: for debugging
  ...
  if (dodebug) then
    WRITE(6,*) "letkf_tools.f90:: After ensmean_grd..."
...
! obs_useidx=0
  ...
  do ij=1,nij1 !STEVE: go through every possible coordinate of the grid in list form...
    ...
    if (verbose .or. dodebug) WRITE(6,*) "ij = ", ij

    if (dodebug .and. ij==dij) then
      WRITE(6,*) "letkf_tools.f90::"
      WRITE(6,*) "Debugging ij = ", ij
      WRITE(6,*) "should be > 4.0 in this region"
      do m=1,nbv
        WRITE(6,*) "ij, m = ", ij, m
        ...
        WRITE(6,*) "gues3d(ij,1,m,iv3d_u) = ", gues3d(ij,1,m,iv3d_u)
        WRITE(6,*) "gues3d(ij,1,m,iv3d_v) = ", gues3d(ij,1,m,iv3d_v)
        WRITE(6,*) "gues3d(ij,1,m,iv3d_t) = ", gues3d(ij,1,m,iv3d_t)
        WRITE(6,*) "gues3d(ij,1,m,iv3d_s) = ", gues3d(ij,1,m,iv3d_s)
      enddo
    endif
    ...
    !---------------------------------------------------------------------------
    ! INPUT-check: zero-valued background members
    !---------------------------------------------------------------------------
    m=1
    if(gues3d(ij,1,m,iv3d_t)==0 .and. gues3d(ij,1,m,iv3d_t)==0 .and. gues3d(ij,1,m,iv3d_t)==0 .and. gues3d(ij,1,m,iv3d_t)==0) then
      WRITE(6,*) "ERROR: background field should not be 0 in all variables at a non-land point."
      WRITE(6,*) "ij, m = ", ij, m
      WRITE(6,*) "kmt1(ij) = ", kmt1(ij)
      WRITE(6,*) "i1(ij) = ", i1(ij)
      WRITE(6,*) "j1(ij) = ", j1(ij)
...
!     if (dodebug) WRITE(6,'(A,I3)') 'ilev = ',ilev
      ...
      if (verbose .or. dodebug) WRITE(6,*) "ilev = ", ilev
      if (verbose .or. dodebug) WRITE(6,*) "mlev = ", mlev
      ...
      do n=1,nv3d
        if (var_local_n2n(n) < n) then
          ...
          if (verbose .or. dodebug) then
            if (nobsl > 0) then
              WRITE(6,*) "letkf_tools.f90::post-obs_local(3d):: Assimilating ", nobsl, " observations."
            else
              WRITE(6,*) "letkf_tools.f90::post-obs_local(3d):: No observations found within local radius."
            endif
          endif
      ...
      if (ilev == 1) then !update 2d variable at ilev=1
        ...
        do n=1,nv2d
          ...
          elseif (var_local_n2n(nv3d+n) < nv3d+n) then
            ...
            if (verbose .or. dodebug) then
              if (nobsl > 0) then
                WRITE(6,*) "letkf_tools.f90::post-obs_local(2d):: Assimilating ", nobsl, " observations."
              else
                WRITE(6,*) "letkf_tools.f90::post-obs_local(2d):: No observations found within local radius."
              endif
            endif
