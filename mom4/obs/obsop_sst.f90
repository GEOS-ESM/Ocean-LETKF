...
!
! DESCRIPTION: 
!   This program acts as the observation operator for Sea Surface Temperature (SST). It inputs observations (yo)
...
! 
! !REVISION HISTORY:
!   Oct 09, 2015 Sivareddy modified to have an exclusive obsop for SST
...
!===============================================================================
  ...
  CHARACTER(slen) :: obsinfile='obsin_sst.dat'    !IN (default)
  CHARACTER(slen) :: guesfile='gues'          !IN (default) i.e. prefix to '.ocean_sbc.res.nc'
  CHARACTER(slen) :: obsoutfile='obsout_sst.dat'  !OUT(default)
  CHARACTER(slen) :: aoerinfile='oer_inp_sst.grd' !IN (default) !SIVA: adoptive observation error arranged on horizontal model grid in a single record
  CHARACTER(slen) :: aoeroutfile='oer_inp_sst.grd'!OUT(default)
...
! LOGICAL :: dodebug = .false.
  ...
  INTEGER :: cnt_nonSST=0	!SIVA: Count non-SST observation records
  ...
  !-----------------------------------------------------------------------------
  CALL set_common_mom4_allvars(id_sst_obs)
  ...
  elseif (obs2nrec==9) then
    ...
    WRITE(6,*) "obsop_sss.f90:: no read_obs option for obs2nrec = ", obs2nrec
    STOP 95
  endif
  ...
  !-----------------------------------------------------------------------------
  ALLOCATE( v2d(nlon,nlat) )
  CALL read_grd_2dvars(guesfile,v2d,id_sst_obs)
  print *, '****************'

  !!STEVE: for adaptive observation error:
  INQUIRE(FILE=aoerinfile, EXIST=oerfile_exists)
  if (oerfile_exists) then
    ALLOCATE(o2d(nlon,nlat))
    CALL read_bingrd_2dvars(trim(aoerinfile),o2d)
  endif
  print *, '****************'
  ...
  oqc=0
  DO n=1,nobs
    !check for SST id. If it is not SST skip it by counting
    if(elem(n) /= id_sst_obs) then
     !SIVA: this can occur if the SST is merged with some other data or due to some error in the path
     cnt_nonSST=cnt_nonSST+1
     CYCLE
    endif
    !---------------------------------------------------------------------------
...
!     oerr(n) = oerr(n)*3.0d0 !*gross_error
    ...
    scale_obs : if (abs(obserr_scaling - 1.0d0) .gt. (TINY(1.0d0)) .or. &
      ...
      SELECT CASE(NINT(elem(n)))
        CASE (id_sst_obs) !surface salt
              if (abs(obserr_scaling - 1.0) > TINY(1.0d0)) &
                oerr(n) = oerr(n) * obserr_scaling
        CASE DEFAULT
              WRITE(6,*) "for n = ", n
                WRITE(6,*) "WARNING: for adaptive observation error, no support for observation type in the obsop_sst: ", elem(n)
        END SELECT
       endif scale_obs
  ...
  DEALLOCATE( elem,rlon,rlat,rlev,odat,oerr,ohx,oqc,obhr,v2d )
