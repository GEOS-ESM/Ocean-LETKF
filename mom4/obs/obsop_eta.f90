...
!
! DESCRIPTION: 
!   This program acts as the observation operator for eta. It inputs observations (yo)
...
! 
! !REVISION HISTORY:
!   Oct 09, 2015 Sivareddy modified to have an exclusive obsop for eta
...
!===============================================================================
  ...
  USE vars_model
  USE common_mom4
  USE params_obs,                ONLY: nobs, id_eta_obs
  USE vars_obs
  USE common_obs_mom4
  USE params_letkf,              ONLY: DO_ALTIMETRY, DO_DRIFTERS
  USE obsop_tools	!SIVA: contains subroutines customized for obsop_*.f90
  ...
  CHARACTER(slen) :: obsinfile='obsin_eta.dat'    !IN (default)
  CHARACTER(slen) :: guesfile='gues'          !IN (default) i.e. prefix to '.ocean_temp_salt.res.nc'
  CHARACTER(slen) :: obsoutfile='obsout_eta.dat'  !OUT(default)
  CHARACTER(slen) :: aoerinfile='oer_inp_eta.grd' !IN (default) !SIVA: adoptive observation error arranged on horizontal model grid in a single record
  CHARACTER(slen) :: aoeroutfile='oer_inp_eta.grd'!OUT(default)
...
! LOGICAL :: dodebug = .false.
  ...
  INTEGER :: cnt_nonETA=0	!SIVA: Count non-ETA observation records
  ...
  !-----------------------------------------------------------------------------
  CALL set_common_mom4_allvars(id_eta_obs)
  ...
  elseif (obs2nrec==9) then
    ...
    WRITE(6,*) "obsop_eta.f90:: no read_obs option for obs2nrec = ", obs2nrec
    STOP 95
  endif
  ...
  CALL read_grd_2dvars(guesfile,v2d,id_eta_obs)
  print *, '****************'
...
!     oerr(n) = oerr(n)*3.0d0 !*gross_error
    ...
    !---------------------------------------------------------------------------
    CALL itpl_2d(v2d,ri,rj,ohx(n))    !(OCEAN)
