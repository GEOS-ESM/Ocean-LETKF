...
!
! DESCRIPTION: 
!   This program acts as the observation operator for ocean salinity. It inputs observations (yo)
...
! 
! !REVISION HISTORY:
!   Oct 09, 2015 Sivareddy modified to have an exclusive obsop for Salinity
...
!===============================================================================
  ...
  CHARACTER(slen) :: obsinfile='obsin_salt.dat'    !IN (default)
  CHARACTER(slen) :: guesfile='gues'          !IN (default) i.e. prefix to '.ocean_temp_salt.res.nc'
  CHARACTER(slen) :: obsoutfile='obsout_salt.dat'  !OUT(default)
  CHARACTER(slen) :: aoerinfile='oer_inp_salt.grd' !IN (default) !SIVA: adoptive observation error arranged on horizontal model in layer-wise records
  CHARACTER(slen) :: aoeroutfile='oer_inp_salt.grd'!OUT(default)
...
! LOGICAL :: dodebug = .false.
  ...
  INTEGER :: cnt_nonSALT=0	!SIVA: Count non-SALT observation records
  ...
  !-----------------------------------------------------------------------------
  CALL set_common_mom4_allvars(id_s_obs)
  ...
  !-----------------------------------------------------------------------------
  ALLOCATE( v3d(nlon,nlat,nlev) )
  CALL read_grd_3dvars(guesfile,v3d,id_s_obs)
  print *, '****************'
  ...
  oqc=0
  DO n=1,nobs
    !check for salinity id. If it is not salinity skip it by counting
    if(elem(n) /= id_s_obs) then
     !SIVA: this can occur if the Salinity is merged with some other data or due to some error in the path
     cnt_nonSALT=cnt_nonSALT+1
     CYCLE
    endif
    !---------------------------------------------------------------------------
...
!               WRITE(6,*) "NEW salt oerr = ", oerr(n)
        ...
        CASE DEFAULT
              WRITE(6,*) "for n = ", n
                WRITE(6,*) "WARNING: for adaptive observation error, no support for observation type in the obsop_salt: ", elem(n)
        END SELECT
       endif scale_obs
    ...
    !---------------------------------------------------------------------------
    CALL itpl_3d(v3d,ri,rj,rk,ohx(n))
  ...
  WRITE(6,*) "cnt_nonSALT =", cnt_nonSALT
  ...
  DEALLOCATE( elem,rlon,rlat,rlev,odat,oerr,ohx,oqc,obhr,v3d )
