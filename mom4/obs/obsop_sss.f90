...
!
! DESCRIPTION: 
!   This program acts as the observation operator for Sea Surface Salinity (SSS). It inputs observations (yo)
...
! 
! !REVISION HISTORY:
!   Oct 09, 2015 Sivareddy modified to have an exclusive obsop for SSS
...
!===============================================================================
  ...
  CHARACTER(slen) :: obsinfile='obsin_sss.dat'    !IN (default)
  CHARACTER(slen) :: guesfile='gues'          !IN (default) i.e. prefix to '.ocean_sbc.res.nc'
  CHARACTER(slen) :: obsoutfile='obsout_sss.dat'  !OUT(default)
  CHARACTER(slen) :: aoerinfile='oer_inp_sss.grd' !IN (default) !SIVA: adoptive observation error arranged on horizontal model grid in a single record
  CHARACTER(slen) :: aoeroutfile='oer_inp_sss.grd'!OUT(default)
...
! LOGICAL :: dodebug = .false.
  ...
  INTEGER :: cnt_nonSSS=0	!SIVA: Count non-SSS observation records
  ...
  !-----------------------------------------------------------------------------
  CALL set_common_mom4_allvars(id_sss_obs)
  ...
  CALL read_grd_2dvars(guesfile,v2d,id_sss_obs)
  print *, '****************'
...
!     oerr(n) = oerr(n)*3.0d0 !*gross_error
    ...
    scale_obs : if (abs(obserr_scaling - 1.0d0) .gt. (TINY(1.0d0)) .or. &
      ...
      SELECT CASE(NINT(elem(n)))
        ...
        CASE DEFAULT
              WRITE(6,*) "for n = ", n
                WRITE(6,*) "WARNING: for adaptive observation error, no support for observation type in the obsop_sss: ", elem(n)
        END SELECT
       endif scale_obs
