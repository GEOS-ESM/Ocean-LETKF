...
!  use vars_obs
!  use common_obs_mom4
! USE obsop_tools	!SIVA: contains subroutines customized for obsop_*.f90
!
!
! DESCRIPTION: 
!   This program acts as the observation operator for ocean temperature. It inputs observations (yo)
...
! 
! !REVISION HISTORY:
!   Oct 09, 2015 Sivareddy modified to have an exclusive obsop for temperature
...
!===============================================================================
  ...
  USE vars_model
  USE common_mom4
  USE params_obs,                ONLY: nobs, id_t_obs
  ...
  CHARACTER(slen) :: obsinfile='obsin_temp.dat'    !IN (default)
  CHARACTER(slen) :: guesfile='gues'          !IN (default) i.e. prefix to '.ocean_temp_salt.res.nc'
  CHARACTER(slen) :: obsoutfile='obsout_temp.dat'  !OUT(default)
  CHARACTER(slen) :: aoerinfile='oer_inp_temp.grd' !IN (default) !SIVA: adoptive observation error arranged on horizontal model in layer-wise records
  CHARACTER(slen) :: aoeroutfile='oer_inp_temp.grd'!OUT(default)
  ...
  REAL(r_size), ALLOCATABLE :: v3dt(:,:,:)
  REAL(r_size), ALLOCATABLE :: v3ds(:,:,:)	!SIVA: Required for potential temperature conversion to in situ
  REAL(r_size), ALLOCATABLE :: o3d(:,:,:)
  REAL(r_size) :: dk,tg,qg
  REAL(r_size) :: ri,rj,rk
  INTEGER :: n

  REAL(r_size),PARAMETER :: & !STEVE: use this to scale the input observations
              obserr_scaling=1.00d0
  REAL(r_size),PARAMETER :: & !STEVE: use this to select random input observations
              obs_randselect=1.00d0 
...
! LOGICAL :: dodebug = .false.
  ...
  INTEGER :: cnt_nonTEMP=0	!SIVA: Count non-temperature observation records
  ...
  !-----------------------------------------------------------------------------
  CALL set_common_mom4_allvars(id_t_obs)
  ...
  elseif (obs2nrec==9) then
    ...
    WRITE(6,*) "obsop_temp.f90:: no read_obs option for obs2nrec = ", obs2nrec
    STOP 95
  endif
  ...
  !-----------------------------------------------------------------------------
  if (DO_INSITU_to_POTTEMP) then  !SIVA: Check point 
    print*, "DO_INSITU_to_POTTEMP case is not yet coded."
    print*,"Exiting obsop_temp.f90....."
    STOP
  endif
  ALLOCATE( v3dt(nlon,nlat,nlev) )
  ALLOCATE( v3ds(nlon,nlat,nlev) )
  CALL read_grd_3dvars(guesfile,v3dt,id_t_obs)
  if ( DO_POTTEMP_to_INSITU .or. DO_INSITU_to_POTTEMP ) then
!SIVA: Salinity along with temperature is required only when DO_POTTEMP_to_INSITU or DO_INSITU_to_POTTEMP are used
   CALL read_grd_3dvars(guesfile,v3ds,id_s_obs)	
  endif
  print *, '****************'
  ...
  if (oerfile_exists) then
    ALLOCATE(o3d(nlon,nlat,nlev))
    CALL read_bingrd_3dvars(trim(aoerinfile),o3d)
  endif
  print *, '****************'
  ...
  oqc=0
  DO n=1,nobs
    !check for SSS id. If it is not SSS skip it by counting
    if(elem(n) /= id_t_obs) then
     !SIVA: this can occur if the SSS is merged with some other data or due to some error in the path
     cnt_nonTEMP=cnt_nonTEMP+1
     CYCLE
    endif
    !---------------------------------------------------------------------------
...
!     oerr(n) = oerr(n)*3.0d0 !*gross_error
    ...
    !---------------------------------------------------------------------------
    scale_obs : if (abs(obserr_scaling - 1.0d0) .gt. (TINY(1.0d0)) .or. &
                    abs(obserr_scaling - 1.0d0) .gt. (TINY(1.0d0)) .or. &
                    abs(obs_randselect - 1.0d0) .gt. (TINY(1.0d0)) .or. &
                    abs(obs_randselect - 1.0d0) .gt. (TINY(1.0d0)) ) then
      ...
      SELECT CASE(NINT(elem(n)))
        CASE (id_t_obs) !temp
          if (abs(obserr_scaling - 1.0) > TINY(1.0d0)) then
...
!               WRITE(6,*) "NEW temp oerr = ", oerr(n)
        ...
        CASE DEFAULT
              WRITE(6,*) "for n = ", n
                WRITE(6,*) "WARNING: for adaptive observation error, no support for observation type in the obsop_temp: ", elem(n)
        END SELECT
       endif scale_obs
    ...
    adapt_obs_error : if (oerfile_exists) then
      SELECT CASE(NINT(elem(n)))
      CASE (id_t_obs) !temp
            oberr = o3d(NINT(ri),NINT(rj),NINT(rk)) 
      CASE DEFAULT
        WRITE(6,*) "for n = ", n
        WRITE(6,*) "WARNING: for adaptive observation error, no support for observation type in the obsop_temp: ", elem(n)
      END SELECT
    ...
    !---------------------------------------------------------------------------
    CALL itpl_3d(v3dt,ri,rj,rk,ohx(n))
    ...
    elseif (DO_INSITU_to_POTTEMP) then  
      STOP "obsop_temp.f90::post-Trans_XtoY::  DO_INSITU_to_POTTEMP case must be coded."
    endif
  ...
  WRITE(6,*) "cnt_nonTEMP =", cnt_nonTEMP
  ...
  DEALLOCATE( elem,rlon,rlat,rlev,odat,oerr,ohx,oqc,obhr,v3dt,v3ds )
